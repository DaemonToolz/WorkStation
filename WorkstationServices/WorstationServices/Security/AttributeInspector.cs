    ...
    public class AttributeInspector : Attribute, IParameterInspector, IOperationBehavior
    {
        ...
        public object BeforeCall(string operationName, object[] inputs)
        {
            ...
            if (name == null || name.Count() == 0 || token == null || token.Count() == 0)
                throw new FaultException<InputValidationFaultContract>(new InputValidationFaultContract() { Error = "No valid data provided" });

          
            if (!UserAccessModel.UserExists(name))
                throw new FaultException<InputValidationFaultContract>(new InputValidationFaultContract() { Error = "User not registered in database" });
        
            if(!Boolean.Parse(ValidateToken("SystemAdministrator", "defaultpassword", token).Result))
                throw new FaultException<InputValidationFaultContract>(new InputValidationFaultContract() { Error = "Invalid Token" });
